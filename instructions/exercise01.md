...
5. Add the technical attributes in the properties panel:
  ...
  2. Expand the General section of the properties panel and change the ID of the process to `PaymentProcess`. Enter `Payment Process` for the Name.
  3. Select the sequence flow that connects the XOR gateway to the "Charge credit card" task. Open the "Condition" section in the properties panel and select Expression as Type. The expression is `${orderTotal > customerCredit}`.
  ...
  5. If not done yet, run Camunda 7 via the `ProcessApplication.java` class.
  6. Now deploy the process model from the Modeler. Press the "Deployment" button (🚀) and enter [http://localhost:8080/engine-rest](http://localhost:8080/engine-rest) as the REST endpoint.
...
In this exercise you have deployed an executable process. Afterwards you started a process instance from the Modeler. You have verified its correct execution history in Cockpit.